...
impl HomeserverConfig {
    ...
    pub fn update_homeserver_config(
        &self,
        ...
        if self.extra_fields.get("listeners").is_none() {
            if let Some(listeners) = combined_config.get_mut(&serde_yaml::Value::from("listeners"))
            {
                ...
                for listener in listeners {
                    ...
                    if !found {
                        ...
                        warn!(
                            ...
                            docker_config.port_mapping[0].guest, docker_config.port_mapping[0].host
                        );
                        listener["port"] = docker_config.port_mapping[0].guest.into()
                    }
                }
            }
        }
