...
lazy_static! {
    ...
    /// The docker tag used for postgresl
    static ref POSTGRES_IMAGE_DOCKER_TAG: OsString = OsString::from_str("postgres").unwrap();

    static ref MX_TEST_DOCKER_NETWORK: String = "mx-tester".to_string();
}
...
fn postgres_root() -> PathBuf {
    std::env::temp_dir().join("mx-tester").join("postgres")
}
...
pub fn down(
    ...
    container_stop("mx-tester_postgres");
    Ok(())
}
